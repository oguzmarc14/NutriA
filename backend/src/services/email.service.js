const { enviarCorreoGmail } = require('./gmail.service')
...
async function enviarInvitacionPaciente({ email, name, nutritionistName, activationToken }) {
  const activationUrl = new URL('/activar-cuenta', process.env.CLIENT_URL || 'http://localhost:5173')
  activationUrl.searchParams.set('token', activationToken)
  const safeName = escapeHtml(name)
  const safeNutritionistName = escapeHtml(nutritionistName)
  const subject = 'Activa tu cuenta de paciente en NutriA'
  const text = [`Hola ${name},`, '', `${nutritionistName} te ha invitado a NutriA.`, 'Crea tu contraseña usando el siguiente enlace:', activationUrl.toString(), '', 'Este enlace vence en 24 horas y solo puede utilizarse una vez.'].join('\n')
  const html = `
    <div style="background:#edf2e7;padding:32px 16px;font-family:Arial,sans-serif;color:#173f34">
      <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:24px;padding:32px;border:1px solid #d5e1db">
        ...
        <p style="margin:28px 0;text-align:center"><a href="${activationUrl.toString()}" style="display:inline-block;background:#246b55;color:#fff;text-decoration:none;font-weight:700;padding:14px 24px;border-radius:12px">Crear mi contraseña</a></p>
  ...
  try {
    await enviarCorreoGmail({ to: email, subject, text, html })
  } catch (error) {
    throw new EmailServiceError(error.message || 'No fue posible enviar la invitación mediante Gmail API.', error)
  }
}
