...
function CardPlanRegistrado({ plan, formatearFecha, numero, calcularTotales }) {
  ...
  return (
    <article className="min-w-0 rounded-2xl border border-[#eadfc8] bg-white/75 p-4 shadow-sm sm:p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={() => setAbierto((actual) => !actual)} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#f5eddb] text-[#8a6840]" aria-label={abierto ? 'Cerrar detalles del plan' : 'Ver detalles del plan'}>
      ...
      {abierto && <div className="mt-4 space-y-3 border-t border-[#efe5d2] pt-4">
        {plan.comidas?.length > 0 ? plan.comidas.map((comida, index) => (
          <div key={comida._id || `${plan._id}-${index}`} className="rounded-xl bg-[#f7f2e7] p-3">
            ...
            {comida.descripcion && <p className="mt-1 text-xs text-slate-500">{comida.descripcion}</p>}
