...
function ModalMedicionGuardada({ mensaje, editada = false, onCerrar, onContinuar }) {
  return (
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/80 bg-white p-6 shadow-2xl">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="titulo-medicion-guardada" className="text-xl font-extrabold text-[#173f34]">{editada ? 'Medición actualizada correctamente' : 'Medidas agregadas correctamente'}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">{mensaje}</p>
            {!editada && <p className="mt-2 text-sm font-semibold text-[#246b55]">¿Quieres ir a registrar las comidas del paciente?</p>}
          </div>
        ...
        {editada ? (
          <button type="button" onClick={onCerrar} className="mt-6 w-full cursor-pointer rounded-xl bg-[#246b55] px-4 py-3 text-sm font-extrabold text-white transition hover:bg-[#1d5947]">Aceptar</button>
        ) : (
