...
function ModalDetalleMedicion({ medicion, onCerrar }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#102d25]/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Detalle de medicion">
      <article className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-[#f8fbf9] p-5 shadow-2xl md:p-7">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#4d816f]">Detalle de medicion</p><h2 className="mt-1 text-2xl font-black text-[#173f34]">{fecha ? new Intl.DateTimeFormat('es-MX', { dateStyle: 'long', timeStyle: 'short' }).format(new Date(fecha)) : 'Sin fecha'}</h2></div>
