...
function obtenerObjetivos(expediente, plan) {
  ...
  return objetivos.length
    ? objetivos.map((objetivo) => nombres[objetivo] || objetivo)
    : plan?.objetivo
      ? [plan.objetivo]
...
function DashboardPaciente({ nombre }) {
  ...
  const planActual =
    perfil?.planes?.find((plan) => plan.activo) || perfil?.planes?.[0];
  const nutriologo = perfil?.paciente?.nutritionist;
  const accesos = [
    {
      nombre: "Mi expediente",
      descripcion: "Consulta tus antecedentes e informacion clinica.",
      icono: ClipboardPlus,
      to: "/mi-expediente",
    },
    {
      nombre: "Mis mediciones",
      descripcion: "Revisa todo tu historial de progreso.",
      icono: Ruler,
      to: "/mis-mediciones",
    },
    {
      nombre: "Mi plan alimenticio",
      descripcion: "Consulta tus comidas e indicaciones actuales.",
      icono: Apple,
      to: "/mi-plan",
    },
  ];
    ...
    <section className="mx-auto max-w-6xl p-5 md:p-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-slate-500">
          Aqui puedes consultar tu progreso y el plan preparado para ti.
        </p>
      </div>
      ...
      <EstadoCargaPaciente loading={loading} error={error} vacio={false}>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <CardResumenPaciente
            icono={UserRound}
            etiqueta="Mi nutriologo"
            valor={nutriologo?.name || "Por confirmar"}
            tono="azul"
          />
          ...
          <CardResumenPaciente
            icono={Goal}
            etiqueta="Objetivos"
            valor={obtenerObjetivos(perfil?.expediente, planActual)}
            detalle="Objetivos actuales de tu seguimiento"
            valorClassName="text-lg leading-snug"
          />
          <CardResumenPaciente
            icono={Utensils}
            etiqueta="Comidas al dia"
            valor={
              perfil?.expediente?.nutricion?.numeroComidasDia ??
              planActual?.comidas?.length ??
              "—"
            }
            detalle="Total registrado en tu expediente clínico"
            tono="arena"
          />
        </div>
        ...
        <div className="mt-5">
          <CardPlanActualPaciente
            plan={planActual}
            onVerPlan={() => navigate("/mi-plan")}
          />
        </div>
